...
const NumberOfBuyers = (props) => {
  ...
  useEffect(() => {
    props.onSavePeople(state.count);
  }, [state.count]);
  ...
  return (
    <Layout>
      ...
      >
        <div className="title-number">Participants</div>
        <p className="sub-title-number">
          {state.count} {state.count > 1 ? "Participants" : "Participant"}
        </p>
      </Number>
      {show && (
        <UserCounter>
          {state.count >= props.maxClient && (
            <p className="max-user-error-message">
              Le nombre de participants est limité à {props.maxClient}
            </p>
          )}
